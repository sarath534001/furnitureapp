...
const authToken = 'a474aa2a4025b199f3ba47ea2f4a7353';
const clien = require('twilio')(accountSid, authToken);
...
const { KeyListInstance } = require('twilio/lib/rest/preview/deployed_devices/fleet/key');
...
async function run() {
    ...
    mong.collection('register').createIndex({mobile:1,password:1})
  ...
  jwt.verify(token,secret,async (err,decoded)=>{
    if(err){
      console.log(err)
       ...
       run().then(k=>{
        req.body.forEach(y=>{
        k.collection('register').updateOne({"mobile":decoded.mobile,"cart.itemname":y.itemname},{$set:{"cart.$.quantity":y.quantity,"cart.$.updatedprice":y.updatedprice}},{returnOriginal:false}).then(u=>{
    //     console.log(u)
        })
       })
       })
